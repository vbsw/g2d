...
import (
	"errors"
	//"fmt"
	"github.com/vbsw/g2d/dummycontext"
	"github.com/vbsw/g2d/window"
...
func (ini tInitializer) SetCData(data unsafe.Pointer) {
	if data != nil {
		propsC := (*C.g2d_props_t)(data)
		MaxTexSize = int(propsC.max_tex_size)
		MaxTexUnits = int(propsC.max_tex_units)
	} else {
		Err = toError(g2dErr1, 0, "")
	}
}
...
func Init() {
	mutex.Lock()
	if !initialized {
		inits := make([]cdata.CData, 3, 3)
		inits[0] = dummycontext.NewInitializer()
		inits[1] = window.NewInitializer()
		inits[2] = new(tInitializer)
		Err = cdata.Proc(inits, 2)
		if Err == nil {
...
func toError(err1, err2 int64, info string) error {
	...
	if err1 > 0 {
		var errStr string
		/* 301 - 400, 1000301 - 1000400 */
		if err1 < 1000000 {
			if err1 > 300 && err1 < 401 {
				errStr = "memory allocation failed"
			}
		} else {
			switch err1 {
			case 1000301:
				errStr = "g2d initializer SetCData has nil"
			case 1000302:
